...
class SoftOptionCritic(object):
    ...
    def reset(self, args):
        ...
        self.option_net = StochasticOptionNetworkKMeans(self.state_dim, self.action_dim, self.option_num,
                                                        self.hidden_dim, self.vat_noise).to(device)
        ...
        for option_idx in range(self.option_num):
            ...
            self.target_actor_net_list.append(
                StochasticActorNetwork(self.state_dim, self.action_dim, self.hidden_dim).to(
                    device))

        for option_idx in range(self.option_num):

            for target_param, param in zip(self.target_actor_net_list[option_idx].parameters(),
                                           self.actor_net_list[option_idx].parameters()):
    ...
    def train_option_autoencoder_kmeans(self, input, action, mean, log_std, target_q_value, predicted_v_value,
                                        sampling_prob):
    ...
    def train_option_autoencoder_kmeansplus(self, input, action, mean, log_std, target_q_value, predicted_v_value,
                                            sampling_prob, index=None):
        option, _, Q_predict = self.softmax_option_target(input)
    ...
    def predict_option_kmeans(self, input, actions):
        if (self.c is None):
            options = torch.randint(0, self.option_num - 1, (int(input.shape[0]), 1))
            return options
        state_action_pair = torch.cat([input, actions], -1)
        results = []
        for idx in range(self.option_num):
