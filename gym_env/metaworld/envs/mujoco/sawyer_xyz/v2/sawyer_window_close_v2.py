...
class SawyerWindowCloseEnvV2(SawyerXYZEnv):
    ...
    def __init__(self):
        ...
        self.init_config = {
            'obj_init_angle': 0.3,
            'obj_init_pos': np.array([0.0, 0.785, 0.16], dtype=np.float32), ## 0
    ...
    def reset_model(self):
        ...
        if self._flag == 1:
            assert self.random_init == False

        if self.random_init:
            self.obj_init_pos = self._get_state_rand_vec()

        # self._target_pos = np.array([-0.1, 0.785, 0.16], dtype=np.float32)
        self._target_pos  = self._get_site_pos('targetPositionClose')
        ...
        self.data.set_joint_qpos('window_slide', 0.1)
